...
public class InjectTransform extends Transform {
    ...
    public void transform(
        ...
        com.kc.gradle.inject.utils.Log.info "==============${project.codelessConfig.pluginName + ' '}transform enter=============="
...
//        Log.info("flavorAndBuildType ${flavorAndBuildType}")
        ...
        if (inputPackages != null) {
            ...
            com.kc.gradle.inject.utils.Log.info "==============@targetPackages = ${targetPackages}=============="
        }
        ...
        inputs.each { TransformInput input ->
            ...
            input.directoryInputs.each { DirectoryInput directoryInput ->
                ...
                Log.i("kcc", "directoryInput->" + directoryInput.file.name);
                ...
                com.kc.gradle.inject.utils.Log.info("项目包含的class文件夹：${directoryInput.file.absolutePath}");
            }
            com.kc.gradle.inject.utils.Log.info('===============================')
            input.jarInputs.each { JarInput jarInput ->

                Log.i("kcc", "jarInput->" + jarInput.file.absolutePath);

                classPaths.add(jarInput.file.absolutePath)
                com.kc.gradle.inject.utils.Log.info("项目包含的jar包：${jarInput.file.absolutePath}");
            }
        }
...
//                Log.info("dest dir  ${dest.absolutePath}")
                ...
                Log.i("kcc", "the file->" + dir + "  dest->" + dest)
                ...
                if (dir) {
                    ...
                    modifyMap.entrySet().each {
                        Map.Entry<String, File> en ->
                            ...
                            com.kc.gradle.inject.utils.Log.info(target.getAbsolutePath());
                            if (target.exists()) {
                                target.delete();
                            }
    ...
    private static void saveModifiedJarForCheck(File optJar) {
        File dir = com.kc.gradle.inject.utils.DataHelper.ext.pluginTmpDir;
    ...
    public static File modifyJarFile(File jarFile, File tempDir) {
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (entryName.endsWith(".class")) {
                    ...
                    if (shouldModifyClass(className)) {
                        modifiedClassBytes = com.kc.gradle.inject.utils.ModifyClassUtil.modifyClasses(className, sourceClassBytes);
                    }
                }
            ...
            com.kc.gradle.inject.utils.Log.info("${hexName} is modified");
    ...
    public static File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            ...
            if (shouldModifyClass(className)) {
                byte[] modifiedClassBytes = com.kc.gradle.inject.utils.ModifyClassUtil.modifyClasses(className, sourceClassBytes);
    ...
    private static String getAppPackageName() {
        ...
        try {
            ...
            com.kc.gradle.inject.utils.Log.info("XmlParser manifestFile: " + manifestFile)
            packageName = new XmlParser().parse(manifestFile).attribute('package')
            com.kc.gradle.inject.utils.Log.info("XmlParser packageName: " + packageName)
        } catch (Exception e) {
            com.kc.gradle.inject.utils.Log.info("XmlParser Exception: " + e.getMessage())
        }
        return packageName
    }
}
